"""
Training script for the SO(3)-equivariant 3D U-Net on QM9 v_ion -> n_r.
...
Usage:
  ...
  python models/equivariant/train.py \
      ...
      --epochs 100 --batch-size 4 --lr 3e-4 --base-channels 8
"""
...
class PhaseADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Per-sample standardization: v_ion has huge range (-127..0)
        # Normalize to ~zero mean, unit variance for stable training
        mu = v_ion.mean()
        sigma = v_ion.std() + 1e-8
        v_ion = (v_ion - mu) / sigma
        inp = torch.from_numpy(v_ion).unsqueeze(0)  # (1, X, Y, Z)
        tgt = torch.from_numpy(n_r).unsqueeze(0)
        return inp, tgt, v_ion.shape  # original shape for mask


def pad_collate(batch):
    """Pad to common shape (multiple of PAD_MULTIPLE), create mask."""
...
def main():
    parser = argparse.ArgumentParser(description="Train equivariant 3D U-Net")
    parser.add_argument("--data", required=True, help="Path to Phase A HDF5")
    parser.add_argument("--experiment-name", default="equivariant_v1")
    ...
    parser.add_argument("--max-freq", type=int, default=2,
                        help="Maximum SO(3) frequency for escnn (L)")
    parser.add_argument("--val-split", type=float, default=0.1)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--amp", action="store_true", default=True)
    parser.add_argument("--no-amp", dest="amp", action="store_false")
    parser.add_argument("--gpu", type=int, default=0)
    parser.add_argument("--last-activation", default="relu",
                        choices=["relu", "softplus", "abs"],
    ...
    # Dirs
    exp_dir = Path("models/experiments") / args.experiment_name
    ...
    print(f"Model: EquivariantUNet3D, {n_params:,} parameters")
    print(f"  base_channels={args.base_channels}, max_freq={args.max_freq}, last_act={args.last_activation}")
