#!/usr/bin/env python3
"""Plot R² scatter for equivariant_v1 model with random axis rotations on 10 molecules.

For each molecule:
  1. Load v_ion (input) and n_r (target) from Phase A dataset
  2. Apply random rotation around x, y, or z axis (random angle)
  3. Apply per-sample standardization to v_ion (matching training)
  4. Pad to multiple of 8, run inference on GPU
  5. Compute R² between predicted n_r and rotated ground-truth n_r
...
def main():
    ...
    with h5py.File(H5_PATH, "r") as f:
        ...
        for i, key in enumerate(chosen):
            ...
            print(f"[{i+1}/{N_MOLS}] {key} ({formula}) | axis={axis_names[axis][0]}, angle={angle:.1f}°")

            v_ion_rot = rotate(v_ion_raw, angle, axes=rot_axes, reshape=False, order=1,
                               prefilter=False, mode="constant", cval=0.0)
            n_r_rot = rotate(n_r_raw, angle, axes=rot_axes, reshape=False, order=1,
                             prefilter=False, mode="constant", cval=0.0)
            ...
            mae = float(np.mean(np.abs(pred - n_r_rot)))
    ...
    for i, (ax, res) in enumerate(zip(axes, results)):
        ...
        ax.set_title(
            f"{res['formula']}  |  {res['axis']}, {res['angle']:.0f}°\n"
    ...
    fig.suptitle(
        "Equivariant 3D U-Net (equivariant_v1): R² under Random Axis Rotation",
        fontsize=13,
        fontweight="bold",
        y=1.01,
    )
    ...
    svg_path = OUT_DIR / "equivariant_rotated_r2.svg"
    fig.savefig(svg_path, dpi=DPI, bbox_inches="tight", format="svg")
    plt.close(fig)
    print(f"\nSaved: {svg_path}", flush=True)

    png_path = OUT_DIR / "equivariant_rotated_r2.png"
    fig2, axes2 = plt.subplots(2, 5, figsize=(28, 11))
    axes2 = axes2.flatten()
    for i, (ax, res) in enumerate(zip(axes2, results)):
        true = res["target"].ravel()
        pred = res["pred"].ravel()
        ax.scatter(true, pred, s=2, alpha=0.25, c="#2166ac")
        vmin = min(float(true.min()), float(pred.min()))
        vmax = max(float(true.max()), float(pred.max()))
        pad = (vmax - vmin) * 0.05
        ax.plot([vmin - pad, vmax + pad], [vmin - pad, vmax + pad], "k--", lw=1.0, alpha=0.6)
        ax.set_xlabel("True n_r", fontsize=9)
        ax.set_ylabel("Pred n_r", fontsize=9)
        ax.set_title(
            f"{res['formula']}  |  {res['axis']}, {res['angle']:.0f}°\n"
    ...
    fig2.tight_layout()
    fig2.savefig(png_path, dpi=DPI, bbox_inches="tight")
    plt.close(fig2)
    print(f"Saved: {png_path}", flush=True)

    summary = {r["key"]: {"formula": r["formula"], "axis": r["axis"], "angle": r["angle"],
                          "r2": r["r2"], "mae": r["mae"]} for r in results}
    for k, v in summary.items():
        print(f"  {k}: {v['formula']} rot={v['axis']}({v['angle']:.1f}°) R²={v['r2']:.6f} MAE={v['mae']:.6f}")
