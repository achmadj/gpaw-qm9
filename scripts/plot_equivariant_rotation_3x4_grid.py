#!/usr/bin/env python3
"""3x4 grid: V_ion / n_pred / n_true / delta_nr for original, 90°, and 180° rotations.

Molecule: gdb_130 (C3H6O2) from QM9 Phase A, equivariant_v1 model.
Rotation axis: y (xz-plane).

Output: SVG (rasterized 3D scatter) + PNG to figures/ dir.
"""
...
KEY = "gdb_110"            # C4H4O, 36×44×40 — balanced grid, 9 atoms
ROT_AXIS = 1               # 1 = y-axis (xz-plane)
ROT_AXIS_NAME = "y (xz-plane)"
PAD_MULTIPLE = 8
SEED = 42
DENSITY_THRESHOLD = 0.01   # threshold for 3D scatter
MAX_POINTS = 8000
ALPHA = 0.55
SIZE = 2.5
DPI = 120                   # lower dpi for SVG to stay reasonable size
...
def infer(model: EquivariantUNet3D, v_ion_raw: np.ndarray, device: torch.device) -> np.ndarray:
    """Per-sample standardize, pad, infer, crop."""
...
def main():
    ...
    model = EquivariantUNet3D(in_channels=1, out_channels=1, base_channels=32,
    ...
    # --- Inference for 3 scenarios ---
    # 0: no rotation
    pred0 = infer(model, v_ion_raw, device)

    # 90°: rotate y-axis (axes 0,2)
    rot_axes = (0, 2)  # y-axis: rotate xz-plane
    ...
    # --- Build 3×4 subplot array ---
    scenarios = [
        ("Original (no rotation)", v_ion_raw, n_r_raw, pred0, (n_r_raw, n_r_raw, pred0 - n_r_raw)),
        (f"Rotated 90° ({ROT_AXIS_NAME})", v_ion_90, n_r_90, pred90, (n_r_90, n_r_90, pred90 - n_r_90)),
        (f"Rotated 180° ({ROT_AXIS_NAME})", v_ion_180, n_r_180, pred180, (n_r_180, n_r_180, pred180 - n_r_180)),
    ]

    fig = plt.figure(figsize=(22, 16))

    for row_idx, (label, vi, nr, pr, _) in enumerate(scenarios):
        diff = pr - nr
    ...
    print("\n=== R² / MAE Summary ===")
    for label, _, nr, pr, _ in scenarios:
